...
%
\section{Solution}
Given equation of the circle $\vec{x}^T\vec{x}-8\myvec{1 & 0}\vec{x}=0$.We know that the general equation of a circle is given by
\begin{align}
  \vec{x}^T\vec{x}+2\vec{u}^T\vec{x}+f=0  
\end{align}
We have $\vec{u} = \myvec{-4 \\ 0}$ and $f = 0$.Thus we have the center and radius as $\vec{c} = -\vec{u} = \myvec{4 \\ 0}$ and $r= \sqrt{\vec{u^T}\vec{u} - f} = 4$ respectively.
Given equation of the parabola $\vec{x}^T\myvec{0&0 \\ 0&1}-4\myvec{0&1}=0$.
